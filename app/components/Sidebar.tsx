...
//       {/* Sidebar */}
//       <aside
//         className={`fixed lg:static top-0 left-0 z-40 w-64 h-screen bg-white border-r border-gray-200 transform ${
  ...
  return (
    <>
      ...
      <div className="lg:hidden fixed top-4 left-4 z-50">
        <button
          onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
          className="p-2 rounded-md bg-white text-gray-700 border shadow-sm focus:outline-none focus:ring-2 focus:ring-[#e6d281]"
        >
          {mobileMenuOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
        </button>
      </div>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:static top-0 left-0 z-40 w-64 h-screen bg-white border-r border-gray-200 transform transition-transform duration-200 ease-in-out font-manrope flex flex-col ${
          mobileMenuOpen ? "translate-x-0" : "-translate-x-full"
        } lg:translate-x-0`}
      >
        {/* Logo Section */}
        ...
        {/* Content Wrapper (nav + logout fixed bottom) */}
